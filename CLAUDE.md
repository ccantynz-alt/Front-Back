...
#### Acknowledgement (no formal confirmation-line ceremony required)

The mandatory reads happen silently. The agent does NOT need to post a stilted "Doctrine confirmed." line in the chat — that ceremony was retired on 2026-04-27 as friction without compensating value (the reads happen regardless; the post was performative).

What the agent MUST still do:
1. Read all the files listed above before any tool call that writes / edits / runs code.
2. If the session is going to advance a 🟡 BUILDING block, name the block in the agent's first message so Craig knows what's in motion.
3. NEVER modify a locked block (🟢 SET or ✅ SHIPPED) without Craig's explicit in-chat authorization. The lock is binding regardless of whether a confirmation line was posted.

Silent code-writing without the reads having happened is the actual doctrine breach. The chat ceremony was a proxy for proving the reads happened; the next session can verify just as well by inspecting whether the session's commits respect the locked-block list.
...
## 5A–5D. ENCYCLOPEDIC REFERENCE — moved to `docs/REFERENCE.md`

The four heavyweight reference sections that previously lived here
(§5A Security & Compliance, §5B Component Architecture, §5C Universal
Device & Integration Support, §5D Environment Variables Roadmap) have
been relocated to `docs/REFERENCE.md` per Craig's 2026-04-27 directive
to trim CLAUDE.md so it stays lean enough to internalise.

That content is **reference, not locked doctrine** — actionable per-session
rules stay here in CLAUDE.md (Iron Rules, Session Protocol, Craig
Authorization Gate, Build-Quality Gate, Parallel Agent Mandate, Drift
Circuit Breaker, Memory Persistence Protocol) and in
`docs/BUILD_BIBLE.md` (locked blocks).

If you need the full security-standards catalog, component-library
picker, integration matrix, or env-var roadmap, see `docs/REFERENCE.md`.
